...
export function SpireEventView({
  building,
  currentView,
  isAdminMode,
  onRequestAdminLogin,
  ...
  const [isBracketFocus, setIsBracketFocus] = useState(
    () => document.body.classList.contains('spire-bracket-focus'),
  );
  ...
  useEffect(() => {
    document.body.classList.toggle('spire-bracket-focus', isBracketFocus);
    window.scrollTo({
      left: 0,
    ...
    return () => {
      document.body.classList.remove('spire-bracket-focus');
    };
  }, [isBracketFocus]);
  ...
  return (
    <>
      ...
      />
      {stamps.map((stamp) => (
        <div key={stamp.id} className="stamp-winner spire-stamp-winner" style={{ left: stamp.x, top: stamp.y }}>
          SPIRE
        </div>
      ))}
      <CelebrationLayer
      ...
      <main className="page spire-page">
        ...
        <div className={`spire-content-grid${currentView === 'admin' ? ' is-admin' : ''}`}>
          <div className="spire-primary-column">
            {currentView === 'public' ? (
              <div className="spire-public-stack" role="tabpanel">
                <RegistrationPanel
                  buttonText={
                  ...
                  statusMessage={
                    tournament.canRegister
                      ...
                      : 'The draw is locked. Registrations are closed for Spire.'
                  }
                  preset={preset}
              ...
              <div className="spire-admin-stack" role="tabpanel">
                <EntriesPanel
                  players={tournament.players}
                  waitingPlayers={tournament.waitingPlayers}
                  countText={countText}
